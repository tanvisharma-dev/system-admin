...
<div class="container-fluid">
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">Edit Reminder</h1>
        ...
        <div class="card-body">
            <form action="{{ route('reminders.update', $reminder) }}" method="POST">
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control @error('title') is-invalid @enderror" id="title" name="title" value="{{ old('title', $reminder->title) }}" required>
                            @error('title')
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="date" class="form-control @error('due_date') is-invalid @enderror" id="due_date" name="due_date" value="{{ old('due_date', $reminder->due_date->format('Y-m-d')) }}" required>
                            @error('due_date')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control @error('type') is-invalid @enderror" id="type" name="type" required>
                                ...
                                @foreach($types as $key => $value)
                                    <option value="{{ $key }}" {{ old('type', $reminder->type) == $key ? 'selected' : '' }}>{{ $value }}</option>
                                @endforeach
                            </select>
                            @error('type')
                                <div class="invalid-feedback">{{ $message }}</div>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control @error('priority') is-invalid @enderror" id="priority" name="priority" required>
                                @foreach($priorities as $key => $value)
                                    <option value="{{ $key }}" {{ old('priority', $reminder->priority) == $key ? 'selected' : '' }}>{{ $value }}</option>
                                @endforeach
                            </select>
                            @error('priority')
                                <div class="invalid-feedback">{{ $message }}</div>
                ...
                <div id="project-field" class="form-group related-fields">
                    ...
                    <select class="form-control select2 @error('related_id') is-invalid @enderror" id="project_id" name="related_id">
                        ...
                        @foreach($projects as $project)
                            <option value="{{ $project->id }}" {{ (old('related_id', $reminder->related_id) == $project->id && old('type', $reminder->type) == 'project') ? 'selected' : '' }}>{{ $project->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div id="seminar-field" class="form-group related-fields">
                    ...
                    <select class="form-control select2 @error('related_id') is-invalid @enderror" id="seminar_id" name="related_id">
                        ...
                        @foreach($seminars as $seminar)
                            <option value="{{ $seminar->id }}" {{ (old('related_id', $reminder->related_id) == $seminar->id && old('type', $reminder->type) == 'seminar') ? 'selected' : '' }}>{{ $seminar->title }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div id="employee-field" class="form-group related-fields">
                    ...
                    <select class="form-control select2 @error('related_id') is-invalid @enderror" id="employee_id" name="related_id">
                        ...
                        @foreach($employees as $employee)
                            <option value="{{ $employee->id }}" {{ (old('related_id', $reminder->related_id) == $employee->id && old('type', $reminder->type) == 'employee') ? 'selected' : '' }}>{{ $employee->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description" rows="4">{{ old('description', $reminder->description) }}</textarea>
                    @error('description')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div id="recurrence-pattern-field" class="form-group" style="display: {{ old('is_recurring', $reminder->is_recurring) ? 'block' : 'none' }}">
                    ...
                    <select class="form-control @error('recurrence_pattern') is-invalid @enderror" id="recurrence_pattern" name="recurrence_pattern">
                        ...
                        @foreach($recurrencePatterns as $key => $value)
                            <option value="{{ $key }}" {{ old('recurrence_pattern', $reminder->recurrence_pattern) == $key ? 'selected' : '' }}>{{ $value }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    <div class="custom-control custom-checkbox">
                        <input type="checkbox" class="custom-control-input" id="is_completed" name="is_completed" value="1" {{ old('is_completed', $reminder->is_completed) ? 'checked' : '' }}>
                        <label class="custom-control-label" for="is_completed">Mark as Completed</label>
                    </div>
                </div>

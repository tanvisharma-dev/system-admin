...
<div class="container-fluid">
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{ route('project-team.update', $projectTeam->id) }}" method="POST">
                @csrf
                @method('PUT')
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control @error('project_id') is-invalid @enderror" id="project_id" name="project_id" required>
                                ...
                                @foreach($projects as $project)
                                    <option value="{{ $project->id }}" {{ old('project_id', $projectTeam->project_id) == $project->id ? 'selected' : '' }}>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control @error('employee_id') is-invalid @enderror" id="employee_id" name="employee_id" required>
                                ...
                                @foreach($employees as $employee)
                                    <option value="{{ $employee->id }}" {{ old('employee_id', $projectTeam->employee_id) == $employee->id ? 'selected' : '' }}>
                ...
                <div class="row mb-3">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control @error('role') is-invalid @enderror" id="role" name="role" required>
                                ...
                                @foreach($roles as $role)
                                    <option value="{{ $role }}" {{ old('role', $projectTeam->role) == $role ? 'selected' : '' }}>
                                        {{ $role }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="date" class="form-control @error('assigned_date') is-invalid @enderror" id="assigned_date" name="assigned_date" value="{{ old('assigned_date', $projectTeam->assigned_date->format('Y-m-d')) }}" required>
                            @error('assigned_date')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control @error('status') is-invalid @enderror" id="status" name="status" required>
                                ...
                                @foreach($statuses as $status)
                                    <option value="{{ $status }}" {{ old('status', $projectTeam->status) == $status ? 'selected' : '' }}>
                                        {{ $status }}
                                    </option>
                                @endforeach
                            </select>

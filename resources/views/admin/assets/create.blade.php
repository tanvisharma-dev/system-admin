            ...
            <div class="card-body">
                <form action="{{ route('assets.store') }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('device_type') is-invalid @enderror" id="device_type" name="device_type" value="{{ old('device_type') }}" placeholder="e.g., Laptop, Monitor" required>
                        @error('device_type')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('asset_condition') is-invalid @enderror" id="asset_condition" name="asset_condition" required>
                            ...
                            <option value="Working" {{ old('asset_condition') == 'Working' ? 'selected' : '' }}>Working</option>
                            <option value="Damaged" {{ old('asset_condition') == 'Damaged' ? 'selected' : '' }}>Damaged</option>
                            <option value="Under Repair" {{ old('asset_condition') == 'Under Repair' ? 'selected' : '' }}>Under Repair</option>
                        </select>
